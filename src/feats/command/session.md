...
4. 使用 **rdpwrap** 实现 [本地远程多用户后台挂机](/tutorial/rdp.html) 的，可能无法正常使用此功能。但是远程多用户功能更加强大且稳定，完全可以覆盖桌面分身的功能，你没有必要使用桌面分身功能。切换rdpwrap状态的方法：[#3409](https://github.com/babalae/better-genshin-impact/issues/3409)
:::
...
1. 直接打开桌面分身功能并启动。**首次可能花费较久的时间初始化，如果报错可以重启软件再试一次，还不行可以重启一次系统后再重试，更多问题请看下方FAQ。尝试所有场景依旧不行，可能和系统环境有关**
2. 首次会要求输入 Windows 登陆凭据，请输入你当前 Windows 登陆用户的用户名密码（用户名一般是 C:\Users 文件夹下属于当前用户的那个文件夹的名称，以这个为准）。历次登陆可能会要求输入登陆凭据，也可能不需要，和 Windows 具体版本和设置有关。
...
1. 仅支持在 Windows 8, Windows Server 2012 及更高版本的**非家庭版系统**中运行（实测部分家庭版系统也可能能用）。BetterGI 本身要求 Windows 10 及以上系统。
...
2. 部分软件不支持这个场景，无法同时在两个桌面中运行。比如 Chrome 浏览器等。**米哈游只能在一个桌面中运行，如果同时开启后，其中的会一直显示等待中**
...
重启 BetterGI 后重试，如果还不行，重启系统后重试。请仔细阅读上面的使用说明和周围的FAQ，可能存在适用于你的场景。
...
3. 用CMD运行一下runas /u:MicrosoftAccount\<微软账户> winver 可能就好了，（例如runas /u:MicrosoftAccount\123456@qq.com winver）然后再重新输入一次密码，相当于用自己的这个账户权限执行一次winver查看系统版本，这样将微软账户密码刷新一下，微软账户云端改过密码后电脑本地没有及时更新导致一个密码本地云端俩头堵。
...
桌面分身内不支持直接允许管理员权限要求的软件，比如米哈游启动器、原神。

你需要使用 BetterGI 桌面分身窗口右上角的 “控制中心” —— “更多” —— “以管理员启动” 功能，找到对应的软件进行启动！

### 能否在桌面分身中运行其他软件？

可以。但是部分软件由于设计原因可能无法同时在两个桌面中启动（比如 Chrome、米哈游启动器）

部分开机自启动的硬件驱动软件也会产生问题，比如一些副屏软件。

需要管理员权限的程序，必须使用 BetterGI 提供的 “以管理员启动” 功能启动。
