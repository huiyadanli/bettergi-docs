...
脚本仓库的更新时间为每天北京时间18:30，偶尔会在其他时间更新，具体以仓库页面左下角时间为准
...
链接：[https://bgi.sh/](https://bgi.sh/)

在使用`在线仓库`订阅脚本时，请确保你的本地仓库已更新为最新版，即左下角的最近更新时间与本地仓库一致。
...
* Javascript脚本：有不同作者编写的各式各样的基于BGI的脚本，花样繁多，强调作者自定义的功能。
* 战斗策略：在运行地图追踪路径时遇到打怪点位时执行的角色战斗逻辑。
* 七圣召唤策略：根据不同角色或关卡执行的七圣召唤打牌策略。

### 3. 选择脚本

选择对应的目录后，目录右侧区域会显示对应目录下的脚本列表。你可以对列表进行搜索或筛选,本地模式下会提供有更新与是否已订阅提示。
...
界面的右侧区域，你会发现一些上文没有提到的按钮，下面将介绍这些按钮的功能。

#### 8.1 最近更新

**仅`本地仓库`可用**  
点击此按钮，你可以查看历史中有更新且未被点击过的脚本，也能清空有更新标记。
...
评论区功能基于`Giscus`插件实现，每个不同的脚本都是不同的评论区（`地图追踪`的每个可以看见的节点都是独立的评论区）。在登录`Github`账号后，可以进行评论或留下小表情。  
> `Github`相关内容请自行使用浏览器搜索  
> 若修改了`主题色`或`语言`，评论区颜色不会立即变化，可以通过刷新页面或关闭后重新打开解决
...
在这里你可以选择你想要的主题色，未来会不定期更新更多的主题色，此部分可自行前往`更新计划`中查看，不过多赘述。  
...
**仅`本地仓库`可用**
与`最近更新`中的`清除更新提示`一致，可以一键消除所有本地的有更新提示。
...
由于`README`的特殊性，此处额外对`README`进行一些说明：
