...
### ❓为什么不拾取/不吃药/不点剧情
为了JS编写时的可配置性，你在实时触发里的所有配置都会在JS脚本运行前关闭，你的实时触发将不会生效，除非JS作者在代码中开启对应配置。  
以下为关闭项（仅对常见问题进行说明）：
  * **自动拾取**：JS中的自动拾取会默认关闭，因此JS跑图时不捡东西是正常的  
  * **自动剧情**：如果进入剧情且作者未开启自动剧情，你将会卡在剧情页面  
  * **快捷传送**：_略_  
  * **自动钓鱼**：_略_  
  * **自动烹饪**：_略_  
  * **自动吃药**：JS中的自动战斗时，红血将不会被检测，角色阵亡回神像是正常的  
  * **游戏加载**：不会检测是否已进入游戏  
