...
地图追踪任务会自动传送到最近的锚点，然后根据小地图开始寻路。系统会根据任务文件的途径点和目标点进行导航。地图追踪任务文件存储在 `\User\AutoPathing`
目录下。
...
这边建议所有的地图追踪任务，都放入调度器中执行，只有调度器能够批量执行任务！
...
在调度器配置组中存在类似的配置，但是不是条件形式，而是强制指定形式，自由度更高，但是至针对调度器配置组生效，且会覆盖地图追踪的默认配置。
