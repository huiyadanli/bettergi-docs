...
| `linnea_mining`     | 莉奈娅挖矿，使用YOLO模型识别矿物并自动瞄准挖矿，具体逻辑见[莉奈娅挖矿策略](#莉奈娅挖矿策略)。`action_params` 格式：`射箭次数,大循环次数`，如 `3` 或 `3,10`。第一个参数为射箭次数（默认1），第二个参数为大循环寻矿次数（默认1）。当大循环次数小于射箭次数时，自动提升为射箭次数。       |
...
取距离聚类中心最近的矿物作为目标（若旋转次数>1则取最靠近的前两个中，靠右侧的矿物，用于对冲旋转的左偏移），转动镜头进行至多7次的瞄准，
若每次瞄准都识别到矿物，但因为识别问题导致来回横跳，会在最后一次发射一发不计入次数上限的箭矢作为兜底
7次循环过程中若未识别到矿物，会退出瞄准状态并将刚才发生的转动偏移补偿回视角，回到初始位置，避免被误识别的目标卡死在固定视角。
若识别不到矿物，会进行视角转动进行寻找。

策略提供了射箭次数与旋转寻矿次数的参数配置，默认值为1,1（都为可选参数，仅填2表示射箭2次，寻矿2次）。
若有矿脉较大，可配置为2次射箭甚至更高。
