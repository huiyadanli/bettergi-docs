...
# 控制文件（control.json）编写手册

## 概述
本说明用于指导编写控制文件（`control.json`），该文件用于定义JSON合并规则。当程序加载追踪文件（`*.json`）时，会根据控制文件中的规则对追踪文件内容进行合并处理。**虽然文件名使用`.json5`后缀（方便编辑器支持语法高亮），但实际只支持标准JSON格式（不支持注释、尾随逗号等JSON5特性）**。
...
## 文件位置
请将控制文件命名为 `control.json5`，并放置在相应的路径追踪目录下，作用于该目录下或指定部分的文件（不支持子目录）。
